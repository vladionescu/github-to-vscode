GITHUB_CODE_DROPDOWN_TEXT = "Code"
GITHUB_CODE_DROPDOWN_SELECTOR = ".react-overview-code-button-action-list ul";
GITHUB_HTTPS_CLONE_LINK_SELECTOR = "#clone-with-https";
GITHUB_SSH_CLONE_LINK_SELECTOR = "#clone-with-ssh";

const codeButton = Array.from(document.querySelectorAll('button')).find(
  button => button.textContent.includes(GITHUB_CODE_DROPDOWN_TEXT)
);
//codeButton.addEventListener('click', () => {
  const targetNode = document.body;
  const observer = new MutationObserver((mutations) => {
    const codeDropdownList = document.querySelector(GITHUB_CODE_DROPDOWN_SELECTOR);
    ...
    if (codeDropdownList) {
      ...
      if (
        codeDropdownList && !document.getElementById("github-to-vscode-clone")
      ) {
        const sshCloneLink = document.querySelector(GITHUB_SSH_CLONE_LINK_SELECTOR);
        const httpsCloneLink = document.querySelector(GITHUB_HTTPS_CLONE_LINK_SELECTOR);
        let repoGitLink = ""
        if (sshCloneLink) {
          repoGitLink = sshCloneLink.value;
        } else if (httpsCloneLink) {
          repoGitLink = httpsCloneLink.value;
        } else {
          console.error("Clone in VSCode: Could not find clone link.");
          return;
        }
        console.log(repoGitLink)
        ...
        cloneInVScodeLink.className =
          "d-flex flex-items-center color-fg-default text-bold no-underline";
    
        cloneInVScodeLink.setAttribute(
          "href",
          `vscode://vscode.git/clone?url=${repoGitLink}`
        );
    
        cloneInVScodeLink.innerHTML = `<svg width="16" height="16" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" class="mr-2">
        ...
        cloneInVScodeNode.appendChild(cloneInVScodeLink);
        codeDropdownList?.appendChild(cloneInVScodeNode);
      } else {
    
        console.error(
          "Clone in VSCode: Code dropdown not found or already injected.", GITHUB_CODE_DROPDOWN_SELECTOR
        );
      }
    }
  });
